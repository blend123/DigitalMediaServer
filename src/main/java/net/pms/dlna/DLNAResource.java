	...
	public Player resolvePlayer(RendererConfiguration renderer) {
		...
		if (resolvedPlayer != null) {
			...
			if (forceTranscode || (preferTranscode && !isSkipTranscode())) {
				if (parserV2) {
					LOGGER.trace("Final verdict: \"{}\" will be transcoded with player \"{}\" with MIME type \"{}\"", getName(), resolvedPlayer.toString(), getMimeType(resolvedPlayer, renderer));
				} else {
...
//		}
	 ...
	 */
	@Nullable
	public MimeType getMimeType(@Nullable Player player, @Nullable RendererConfiguration renderer) { //TODO: (Nad) Here
		if (player == null) {
			player = this.player;
		}
		...
		if (LOGGER.isTraceEnabled()) {
			...
			if (renderer != null) {
				LOGGER.trace("Resolved MIME type {} for {} for {}", mimeTypeString, renderer, this);
			} else {

...
//			try {
				...
				// XXX Should use parseDuration() and "Duration" instead, but that returns null instead of 0.0 if it can't be parsed whose effects must be checked
...
//			}
	...
	protected static class ParseLogger {
		...
		public void logGeneral(File file) {
			...
			appendString("Enc Date", MI.Get(StreamType.General, 0, "Encoded_Date"), first, true, true);
		}
		...
		public void logGeneralColumns(File file) {
			...
			appendStringNextColumn(generalColumns, "Enc Date", MI.Get(StreamType.General, 0, "Encoded_Date"), true, true);
		}
		...
		public void logVideoTrack(int idx) {
			...
			first &= !appendString("Mime", MI.Get(StreamType.Video, idx, "InternetMediaType"), first, true, true);
		...
		public void logVideoTrackColumns(int idx) {
			...
			appendStringNextColumn(streamColumns, "Mime", MI.Get(StreamType.Video, idx, "InternetMediaType"), true, true);
			Double durationSec = parseDuration(MI.Get(StreamType.Video, 0, "Duration"));
			if (durationSec != null) {
		...
		public void logAudioTrack(int idx) {
			...
			first &= !appendString("Mime", MI.Get(StreamType.Audio, idx, "InternetMediaType"), first, true, true);
		...
		public void logAudioTrackColumns(int idx) {
			...
			appendStringNextColumn(streamColumns, "Mime", MI.Get(StreamType.Audio, idx, "InternetMediaType"), true, true);
			Double durationSec = parseDuration(MI.Get(StreamType.Audio, 0, "Duration"));
			if (durationSec != null) {
				appendStringNextColumn(streamColumns, "Duration", StringUtil.formatDLNADuration(durationSec), false, true);
			}
		...
		public void logImage(int idx) {
			...
			first &= !appendString("Mime", MI.Get(StreamType.Image, idx, "InternetMediaType"), first, true, true);
		...
		public void logImageColumns(int idx) {
			...
			appendStringNextColumn(streamColumns, "Mime", MI.Get(StreamType.Image, idx, "InternetMediaType"), true, true);
